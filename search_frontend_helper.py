...
def BM25(search_query, index, k, b,avg_doclen,N, threshHold=73):
...
def combined_Search(title_index, body_index, anchor_index, query, k, b, w_anc, w_body, w_title, avg_doclen,
                               N):
    filtered_query = tokenize_and_remove_sw(query)
    ...
    if len(filtered_query) == 1:
        for doc_id, score in anchor_scores.items():
            if doc_id in pr and doc_id in pv_dct :
                if pr_dct[doc_id] > pv_dct[doc_id]:
                    combined_scores[doc_id] += (w_body * score * w_anc * pr[doc_id]) / (score + pr[doc_id])
                else:
                    combined_scores[doc_id] += (w_body * score * w_anc * pr[doc_id] * pv_dct[doc_id]) / (score + pr[doc_id])
            elif doc_id in pr:
                combined_scores[doc_id] += (w_body * score * w_anc * pr[doc_id]) / (score + pr[doc_id])
    else:
        ...
        for doc_id, score in body_scores.items():
            if doc_id in pr:
              combined_scores[doc_id] += (w_body * score * pr[doc_id] ) / (score + pr[doc_id])
        for doc_id, score in anchor_scores.items():
            if doc_id in pr:
